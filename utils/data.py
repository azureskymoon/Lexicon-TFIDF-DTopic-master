...
class Data:
    def __init__(self): 
        ...
        #产生候选词语。利用预训练词向量生成词语树，
        # 每一个句子从开始位置进行最大匹配的原则，匹配当前字符下的所有词语
        self.word_alphabet = Alphabet('word')
        self.biword_alphabet = Alphabet('biword')
        self.char_alphabet = Alphabet('character')
        self.label_alphabet = Alphabet('label', True)
        self.gaz_lower = False
        ##Gazetteer类初始化，作用是保存词向量所有词汇，主要初始化了一个Trie，
        # 这个就是为了存储词语的树，是为了在训练数据每一个句子中有多少个词语
    ...
    def build_gaz_alphabet(self, input_file, count=False):
        ...
        for line in in_lines:
            if len(line) > 3:
                ...
                # 以下匹配一个句子中有多少个单词并且加入到self.gaz_alphabet中，可以说此变量是个字典，包含了所有训练数据预测数据匹配词语的个数
