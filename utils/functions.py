        ...
        if len(line) > 2 and len(line) != 3:
            ...
            if ((max_sent_length < 0) or (len(words) < max_sent_length)) and (len(words)>0):
                ...
                for idx in range(w_length):
                    ...
                    for w in range(len(matched_Id)):
                        ...
                        if matched_length[w] == 1:  ## Single
                            gazs[idx][3].append(matched_Id[w])  ##单个字的位置 如 科 对应单独的一个子  [[2],[],[],[3]]
                            gazs_count[idx][3].append(1)        ##[[4],[],[],[1]]
                            gaz_char_Id[idx][3].append(gaz_chars)  ##[[[2，3]],[],[],[[2]]]
                        else:
                            ...
                            gazs[idx+wlen-1][2].append(matched_Id[w])  ## End  每个字对应的词 以及字在词中的位置 以及字对应字典下标位置
                            # 比如 科 对应 科学 分别位于科的首部，以及学的尾部 [[[2],[],[],[]],[[],[],[2],[]]]
                            gazs_count[idx+wlen-1][2].append(gaz_count[matched_Id[w]])  ## 词对应词典中的频率，即词频
                            gaz_char_Id[idx+wlen-1][2].append(gaz_chars) ## 词对应的两个字的下标
                ...
                for idx in range(w_length):
                    ...
                    for label in range(4):
                        label_len = len(gazs[idx][label])    # 字对应词的数量 比如 科 对应 科技 一个词 len = 1
                        count_set = set(gazs_count[idx][label])   #  词频
...
def build_pretrain_embedding(embedding_path, word_alphabet, embedd_dim=100, norm=True):    
    ...
    pretrain_emb = np.empty([word_alphabet.size(), embedd_dim])   #tensor[3350,50]
    perfect_match = 0
    case_match = 0
    not_match = 0
    ...
    for word, index in word_alphabet.instance2index.items():
        if word in embedd_dict:
            ...
            perfect_match += 1
        elif word.lower() in embedd_dict:  #大写变成小写
            if norm:
