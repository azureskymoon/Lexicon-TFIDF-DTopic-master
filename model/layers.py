...
def attention(query, key, value, mask=None, dropout=None):
    ...
    """
     ...
     """
    if mask is not None:
        # scores = scores.masked_fill(mask == 0, -1e9)  #将 pad 对应的 score 取值为负无穷, 以此来减小 pad 的影响
...
class MultiHeadedAttention(nn.Module):
    def __init__(self, h, d_model, dropout=0.1):
        ...
        #h和d_model分别表示注意力头的个数，以及模型的隐层单元数
        super(MultiHeadedAttention, self).__init__()
        assert d_model % h == 0
        # We assume d_v always equals d_k
        self.d_k = d_model // h
        self.h = h
        #clone(x, N)即为深拷贝N份，这里定义了4个全连接函数，实际上是3 + 1，其中的3个分别是Q、K和V的变换矩阵，最后一个是用于最后将多头concat之后进行变换的矩阵。
