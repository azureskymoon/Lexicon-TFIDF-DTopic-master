...
# args  各种初始化参数
# bichar_emb char_emb 字向量、词向量  ctb.50d 词向量
# data
# word_alphabet（1837、训练集单个字的个数）
# biword_alphabet(21407、每相邻两个字组成一个词)
# biword_count(21406、biword_alphabet中每个词出现的次数)
# char_alphabet(1894、训练集总共有1894个字)
# Gazetteer(704368、词典)
# gaz_alphabet(11223、每个字对应的词语或字的组成)
# gaz_count(11223、gaz_alphabet每个词或字对应的次数)
...
def data_initialization(data, gaz_file, train_file, dev_file, test_file):
    ...
    #匹配一个训练语句中能够匹配多少个词汇信息
    data.build_gaz_alphabet(train_file, count=True)   #利用字符串的拼接在词向量中的个数
...
def evaluate(data, model, name):
    ...
    acc, p, r, f = get_ner_fmeasure(gold_results, pred_results, "BIO")
...
def train(data, save_model_dir, seg=True):
    ...
    for idx in range(data.HP_iteration):
        ...
        ## set model in train model  把模型中参数的梯度设为0
        model.train()
        model.zero_grad()
        ...
        total_batch = train_num // batch_size + 1
        q = numpy.random.uniform(low=-1, high=1, size=(1, 50))
        k = numpy.random.uniform(low=-1, high=1, size=(4, 50))

        for batch_id in range(total_batch):##每次处理一句话
            start = batch_id * batch_size
            end = (batch_id + 1) * batch_size
            if end > train_num:
                end = train_num
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train', default="data/Medical2018/train.char.bmes")
    parser.add_argument('--dev', default="data/Medical2018/dev.char.bmes")
    parser.add_argument('--test', default="data/Medical2018/test.char.bmes")
    ...
    char_emb = r"E:\NLP\Code\LexiconAugmentedNER-master\data\gigaword_chn.all.a2b.uni.ite50.vec"
    bichar_emb = r"E:\NLP\Code\LexiconAugmentedNER-master\data\gigaword_chn.all.a2b.bi.ite50.vec"
    gaz_file = r"E:\NLP\Code\LexiconAugmentedNER-master\data\ctb.50d.vec"
    ...
    if status == 'train':
        if os.path.exists(save_data_name):
            ...
            data.build_word_pretrain_emb(char_emb)  #返回匹配到的字向量的数目以及未匹配率:00v/(perfect match + ovv)
